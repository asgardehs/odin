...
-- ============================================================================
-- MIGRATION-RUNNER TICKET (still deferred, ALTER path only now)
-- ============================================================================
-- For FRESH installs, the 4 + 6 new ITA columns land directly via the
-- CREATE TABLE definitions above (establishments + incidents).
--
-- For EXISTING installed databases, a separate migration runner (tracked
-- as its own ticket) must issue the equivalent ALTER TABLE ADD COLUMN
-- statements with pragma_table_info idempotency guards. That ticket is
-- outside this file's scope.
--
-- Views for the ITA export live in sql/views/osha_ita.sql and are re-run
-- on every odin startup (see internal/database/views.go). Do not add
-- CREATE VIEW statements in this file — they'd be stuck behind the
-- applied-once migration guard and wouldn't update on code change.
